"""
Evolutionary Discovery of Optimal Signal Correlation
=====================================================

Uses a genetic algorithm to DISCOVER that spatially-correlated
signals are optimal for soft-body locomotion — without human
prior knowledge.

Each genome encodes per-particle oscillator parameters:
  - phase_offset: initial phase of the oscillator
  - freq_scale: frequency multiplier
  - amplitude_x, amplitude_y: force amplitudes

The GA evolves these parameters to maximize forward displacement.
The key scientific question: will evolution independently discover
spatial correlation (nearby particles moving in sync)?

CPU-only. With multiprocessing (24 cores), runs in ~5 minutes.

Usage:
    python src/evolve_locomotion.py
"""
...
def evaluate_fitness(genome, seed=None):
    """Fitness = net forward displacement. Penalize for body disintegration."""
    if seed is not None:
        np.random.seed(seed)
    ...
    # Penalty for body disintegration (particles too spread out)
    ...
    fitness = displacement - spread_penalty - ground_penalty
    return fitness
...
def visualize(best_genome, history, total_time):
    ...
    fig.suptitle("Evolutionary Discovery of Locomotion Patterns\n"
                 f"GA discovered spatial correlation (r={corr:.3f}) without prior knowledge",
    ...
    plt.tight_layout()
    path = os.path.join(OUTPUT_DIR, "evolution_discovery.png")
...
def main():
    ...
    # Save results
    results = {
        "experiment": "Evolutionary Discovery of Locomotion Patterns",
    ...
    log_path = os.path.join(RESULTS_DIR, "evolution_log.json")
